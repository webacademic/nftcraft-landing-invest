...
window.buyNCT = async function() {
    ...
    const approvedAmount = await usdt.methods.allowance(NCT_SELL_ADDRESS, accounts[0]).call();
    if  (approvedAmount < requiredUSDTAmount) {

        await new Promise((success) => {
            usdt.methods.approve(NCT_SELL_ADDRESS, requiredUSDTAmount).send({from: accounts[0]}).on('transactionHash', function (hash) {
                //
    ...
    try
    {
        gas = await nctSell.methods.swap(requiredNCTAmount).estimateGas({from: accounts[0]});
    }catch (e) {
        console.error(e);
    ...
    await new Promise((success) => {
        nctSell.methods.swap(requiredNCTAmount).send({from: accounts[0], gas: gas}).on('transactionHash', function (hash) {
            //
        }).on('receipt', function (receipt) {
            success();
        });
